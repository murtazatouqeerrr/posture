...
async function loadReportsView() {
    ...
    try {
        // Fetch all report data including invoices and subscriptions for counts
        ...
        if (!leadsResponse.ok || !conversionResponse.ok || !revenueResponse.ok || !analyticsResponse.ok || !invoicesResponse.ok || !subscriptionsResponse.ok) {
        ...
        // Add invoice and subscription counts
        const invoices = await invoicesResponse.json();
        const subscriptions = await subscriptionsResponse.json();
        
        reportsData.financialAnalytics.totalInvoices = invoices.length;
        reportsData.financialAnalytics.activeSubscriptions = subscriptions.length;
        
        // Calculate invoice status counts
        const paidInvoices = invoices.filter(inv => inv.status === 'Paid').length;
        const overdueInvoices = invoices.filter(inv => inv.status === 'Overdue').length;
        
        reportsData.financialAnalytics.paidInvoices = paidInvoices;
        reportsData.financialAnalytics.overdueInvoices = overdueInvoices;
