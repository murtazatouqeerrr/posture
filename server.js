...
// ADD MISSING ENDPOINTS FOR NEW MODULES
app.get('/api/nudge/history', (req, res) => {
    db.all(`SELECT * FROM automated_emails WHERE email_type IN ('low_sessions_warning', 'package_renewal', 'dormant_reactivation', 'feedback_request') ORDER BY sent_at DESC`, [], (err, rows) => {
        if (err) {
            res.json([]);
            return;
        }
